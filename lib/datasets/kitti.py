...
class KITTI(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.split!='test':
            ...
            vis_depth = np.zeros((self.max_objs, roi_size, roi_size), dtype=np.float32)
            mh_vis_depth = np.zeros((self.max_objs, window_cnt), dtype=np.float32)
            ...
            # if False:
                ...
                for i in range(object_num_temp):
                    ...
                    mh_vis_depth[i + object_num] = depth[i + object_num]
