...
class MonoMH(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None):
        ...
        self.multi_vis_depth = nn.Sequential(nn.Conv2d(channels[self.first_level]+2+self.cls_num, self.head_conv, kernel_size=3, padding=1, bias=True),
                                       nn.LeakyReLU(inplace=True),
                                       nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.multi_vis_depth.apply(weights_init_xavier)
    ...
    def get_roi_feat_by_mask(self,feat,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids):
        ...
        if num_masked_bin!=0:
            ...
            roi_feature_masked = roi_align(feat,scale_box2d_masked,[roi_size,roi_size])
            ...
            coord_maps = torch.cat([torch.cat([coords_in_camera_coord[:,1:2]+i*(coords_in_camera_coord[:,3:4]-coords_in_camera_coord[:,1:2])/(roi_size-1) for i in range(roi_size)],-1).unsqueeze(1).repeat([1,roi_size,1]).unsqueeze(1),
                                torch.cat([coords_in_camera_coord[:,2:3]+i*(coords_in_camera_coord[:,4:5]-coords_in_camera_coord[:,2:3])/(roi_size-1) for i in range(roi_size)],-1).unsqueeze(2).repeat([1,1,roi_size]).unsqueeze(1)],1)
            ...
            roi_feature_masked = torch.cat([roi_feature_masked,coord_maps,cls_hots.unsqueeze(-1).unsqueeze(-1).repeat([1,1,roi_size,roi_size])],1)
            ...
            multi_vis_depth = self.multi_vis_depth(roi_feature_masked).squeeze(1)
            multi_vis_depth = (-multi_vis_depth).exp()
            multi_vis_depth = multi_vis_depth * scale_depth.unsqueeze(-1).unsqueeze(-1)
            ...
            res['multi_vis_depth'] = multi_vis_depth
            ...
            res['vis_depth'] = torch.zeros([1,roi_size,roi_size]).to(device_id)
            res['vis_depth_uncer'] = torch.zeros([1,roi_size,roi_size]).to(device_id)
            res['multi_vis_depth'] = torch.zeros([1,roi_size,roi_size]).to(device_id)
